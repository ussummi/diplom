...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    fetch("/storesData.json")
      .then((res) => res.json())
      .then((data) => {
        const foundStore = data.find((s: any) => s.ID.toString() === storeId);
        setStore(foundStore);
      });
  }, [storeId]);
  ...
  const handlePlaceOrder = () => {
    ...
    const orderId = timestamp; //ingej bga shaltgaan n ayulgui baidal
    const updatedOrders = [
      ...savedOrders,
      {
        id: orderId,
  ...
  return (
    <main className="px-6 py-8 flex flex-col lg:flex-row gap-6 sm:gap-8 md:gap-12 overflow-x-hidden">
      <Toaster position="top-center" />
      <aside className="w-full sm:w-full lg:w-3/8 h-auto rounded md:p-4 overflow-y-auto max-h-screen">
        ...
        <div className="mt-4 border rounded-xl bg-white border-black p-4">
          ...
          <ul className="list-disc list-inside text-sm">
            {store.promotions?.length ? (
              store.promotions.map((promo: string, idx: number) => (
                <li key={idx}>{promo}</li>
              ))
            ) : (
      ...
      <section className="w-full sm:w-full lg:w-5/8 flex flex-col gap-6 overflow-x-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 overflow-x-auto">
          {store.products.map((product: any) => (
            <div
              key={product.id}
              className="border rounded-xl bg-white border-black shadow p-4 text-center cursor-pointer hover:shadow-lg transition-all duration-300 overflow-hidden"
        ...
        <div className="border rounded-xl bg-white border-black p-4 overflow-x-auto">
        ...
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 overflow-auto backdrop-blur-sm">
          <div className="relative border border-black bg-white rounded-lg p-6 w-80 text-center overflow-auto">
            ...
            <button
              ...
              className="absolute top-2 right-2 bg-white text-red-600 rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-gray-300 transition-colors duration-300 cursor-pointer"
            >
              ✖
            </button>
